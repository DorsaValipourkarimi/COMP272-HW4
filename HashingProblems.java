...
class HashingProblems {
    ...
    public double getAverage(HashMap<Integer, Integer> map, int[] array) {
        ...
        // Initialize result in case no matches was found
        double result = 0.0 / 0.0;

        // To store the sum of matching values
        double sum = 0;

        // To count how many matches we find
        int count = 0;
        // Loop through each number in the array
        for (int key : array) {
            // Check if the number is a key in the map
            if (map.containsKey(key)) {
                // Add the corresponding value from the map to the sum
                sum += map.get(key);
                // Increase the count of matches found
                count++;
            }
        }

        // If count is not zero, calculate the average
        if (count != 0) {
            result = sum / count;
        }
    ...
    public ArrayList<String> odd(HashMap<Integer, String> map) {
        ...
        for (Integer key : map.keySet()) {
            ...
            if (key % 2 != 0) {
                // If so, add the corresponding value to the list
    ...
    public int twoSums(int[] numbers, int k) {
        ...
        // HashSet to store previously seen numbers
        HashSet<Integer> s = new HashSet<>();

        // Counter to track the number of valid pairs
        int count = 0;

        // Loop through each number in the array
        for (int num : numbers) {
            // Check if a number exists that is smaller by k
            if (s.contains(num - k)) {
                count++; // Increment count if found
            }

            // Check if a number exists that is larger by k
            if (s.contains(num + k)) {
                count++; // Increment count if found
            }
